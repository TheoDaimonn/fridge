...
1)Заполнить список того, что у вас есть, вбивая поочередно все компоненты(лед, еда, фрукты, специи. Желательно указывать все, иначе я не смогу подобрать для вас коктейль)
...
P.P.S. Вы можете оформить подписку на наш сервис за 50р, чтобы обеспечить себе пожиззненный PRO status, добавляющий вас в особый канал спонсоров и снимающий ограничение на количество использований бота в день
P.P.P.S. Не забудьте про лЁд(именно так. в связи с проблемой с системой его указание обязательно для любых холодных коктейлей)""", reply_markup=markup)
        tyty = open('currentingr.txt', 'r', encoding='utf-8').readline().split('|')
        bot.send_message(message.chat.id, f'текущий список возможных в использовании ингридиентов:\n' + ', '.join(tyty))
